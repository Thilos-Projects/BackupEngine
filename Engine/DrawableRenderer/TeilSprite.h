#ifndef _TeilSprite_H_
#define _TeilSprite_H_

#include "DrawableRenderer.h"
#include "../other/Transform.h"

class TeilSprite : public Drawable
{
private:
    static ComponentRegister<TeilSprite> reg;
public:
    struct TeilSpriteVertex
    {
        XMFLOAT3 pos;
        XMFLOAT2 tex0;
    };
    ...
    virtual bool load(DrawableRenderer* renderer)
    {
        ...
        HRESULT hr = ::D3DX11CreateShaderResourceViewFromFile(renderer->m_pD3DDevice, tPath.c_str(), 0, 0, &m_pColorMap, 0);
        if (FAILED(hr)) {
        ...
        textureDesc.Filter = D3D11_FILTER::D3D11_FILTER_MIN_MAG_MIP_LINEAR;
        ...
        TeilSpriteVertex vertices[] =
        {
            { XMFLOAT3(halfWidth,  halfHeight, 1.0f), XMFLOAT2((float)m_spriteRect[2], (float)m_spriteRect[3]) },
            { XMFLOAT3(halfWidth, -halfHeight, 1.0f), XMFLOAT2((float)m_spriteRect[2], (float)m_spriteRect[1]) },
            { XMFLOAT3(-halfWidth, -halfHeight, 1.0f), XMFLOAT2((float)m_spriteRect[0], (float)m_spriteRect[1]) },

            { XMFLOAT3(-halfWidth, -halfHeight, 1.0f), XMFLOAT2((float)m_spriteRect[0], (float)m_spriteRect[1]) },
            { XMFLOAT3(-halfWidth,  halfHeight, 1.0f), XMFLOAT2((float)m_spriteRect[0], (float)m_spriteRect[3]) },
            { XMFLOAT3(halfWidth,  halfHeight, 1.0f), XMFLOAT2((float)m_spriteRect[2], (float)m_spriteRect[3]) }
        };
        ...
        vertexDesc.ByteWidth = sizeof(TeilSpriteVertex) * 6;
    ...
    {
        UINT stride = sizeof(TeilSpriteVertex);
        UINT offset = 0;
    ...
    virtual void set(SerializedObject& from) {
        from >> m_spriteRect[3] >> m_spriteRect[2] >> m_spriteRect[1] >> m_spriteRect[0] >> m_shaderPath >> m_texturePath;
    };
    ...
    virtual SerializedObject serialize(SerializedObject& inTo) {
        inTo << m_texturePath << m_shaderPath << m_spriteRect[0] << m_spriteRect[1] << m_spriteRect[2] << m_spriteRect[3];
        inTo << typeid(TeilSprite).name();
        return inTo;
    };

    std::string m_texturePath;
    std::string m_shaderPath;
    float m_spriteRect[4] = { 0.0f,0.0f,1.0f,1.0f };
...
ComponentRegister<TeilSprite> TeilSprite::reg(typeid(TeilSprite).name());
#endif