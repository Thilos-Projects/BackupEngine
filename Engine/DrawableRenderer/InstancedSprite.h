...
class InstancedSprite : public Drawable
{
private:
    static ComponentRegister<InstancedSprite> reg;
public:
    struct InstancedSpriteVertex
    {
        XMFLOAT3 pos;
        XMFLOAT2 tex0;
    };
    ...
    InstancedSprite()
    {
        m_shaderPath = "";
        m_texturePath = "";
        m_instancen = std::vector<Transform*>();
    ...
    virtual bool load(DrawableRenderer* renderer)
    {
        ...
        InstancedSpriteVertex vertices[] =
        {
        ...
        vertexDesc.ByteWidth = sizeof(InstancedSpriteVertex) * 6;
    ...
    {
        UINT stride = sizeof(InstancedSpriteVertex);
        UINT offset = 0;
        ...
        for (int i = 0; i < m_instancen.size(); i++) {
            ...
            XMMATRIX world = m_instancen[i]->GetWorldMatrix();
    ...
    virtual void set(SerializedObject& from) {
        ...
        for (int i = 0; i < size; i++) {
            from >> temp;
            m_instancenID.push_back(temp);
        }
        from >> m_shaderPath >> m_texturePath;
    };
    virtual void afterSet() {
        for (int i = 0; i < m_instancenID.size(); i++)
            m_instancen.push_back((Transform*)ObjectStorrage::getInstance()->getById(m_instancenID[i])->findComponents<Transform>()[0]);
    };
    ...
    virtual SerializedObject serialize(SerializedObject& inTo) {
        ...
        int size = m_instancenID.size();
        for (int i = size - 1; i > -1; i--)
            inTo << m_instancenID[i];
        inTo << size;
        inTo << typeid(InstancedSprite).name();
        return inTo;
    };
    ...
    std::vector<Transform*> m_instancen;
    std::vector<int> m_instancenID;
...
ComponentRegister<InstancedSprite> InstancedSprite::reg(typeid(InstancedSprite).name());
#endif